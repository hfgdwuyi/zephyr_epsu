 ...
 *
 * Copyright (c) 1997-2013 port GmbH Halle (Saale)
 *------------------------------------------------------------------
 * $Id: cdriver.c,v 2.93 2013/08/26 12:24:17 hes Exp $
 *
 * modification history
 * --------------------
 * $Log: cdriver.c,v $
 * Revision 2.93  2013/08/26 12:24:17  hes
 * interchanged pointer check and allocation for pCOB in function Insert_TX_Request()
 *
 * Revision 2.92  2013/03/25 10:49:53  hes
 * printf call changed to PRINTF,
 * formatting adapted
 *
 * Revision 2.91  2013/01/14 12:16:15  hes
 * date in copyright changed
 *
 * Revision 2.90  2012/11/19 08:26:17  hes
 * corrected
 *
 * Revision 2.89  2012/10/10 12:28:08  oe
 * - changed some PRINTF from macro to printf (in debug code)
 * - removed some spelling errors
 *
 * Revision 2.88  2012/02/16 10:37:48  hes
 * date in copyright changed
 *
 * Revision 2.87  2011/04/27 12:57:16  ro
 * comment
 *
 * Revision 1.1  2010/10/19 08:00:15  ro
 * v4.4.43
 *
 * Revision 2.85  2009/08/06 07:52:02  ro
 * LPC2000 specific function adapted to Multiline
 *
 * Revision 2.84  2009/07/20 10:20:02  ro
 * CONFIG_DRIVER_FAST_SORT implemented
 * (can be set independent of CONFIG_FAST_SORT)
 *
 * Revision 2.83  2009/02/20 09:54:23  ro
 * validCobId() without active Fast Sort -> List mode,
 * ignore COBs with the wrong type.
 * Need for Services, that can be switched between RX and TX
 * (e.g. Flying master).
 *
 * set COB_NUMBER_CNT for the no-global-vars mode.
 *
 * Revision 2.82  2008/08/14 10:18:17  boe
 * correct returnvalue at init_canDriverPtr() for multiline
 *
 * Revision 2.81  2008/08/13 13:58:28  boe
 * add function and use access macros (GL_DRV_PREDCY) for usage of dynamic memory
 *
 * Revision 2.80  2008/01/29 16:31:47  ro
 * new Definitions:
 * - CO_MEM_QUICKRAM
 * - CO_MEM_RAM
 * - CO_MEM_CAN
 *
 * Bug:
 * initCobList() for Multiline/Redundancy mode corrected
 *
 * Revision 2.79  2007/11/15 13:17:49  ro
 * include of stdio removed
 *
 * Revision 2.78  2007/09/13 10:23:48  ro
 * small correction in the experimental part CONFIG_INDEXLIST_OPT
 *
 * Revision 2.77  2007/06/29 11:40:13  ro
 * some more comments added
 *
 * Revision 2.76  2007/05/24 08:18:29  ro
 * Feature:
 * Insert_TX_Request() check for pCOB == NULL added
 * (e.g. needed for polling GetNext_TX_Request()
 * by calling Transmit_COB() with NULL parameter)
 *
 * Documentation:
 * comment of Insert_TX_Request() changed
 *
 * Revision 2.75  2007/03/08 13:41:36  ro
 * experimental version of CONFIG_INDEXLIST_OPT corrected
 *
 * Revision 2.74  2007/01/22 14:40:25  ro
 * first version of getSortRxCOB() only for LPC21xx family, yet
 *
 * Revision 2.73  2007/01/19 13:40:09  ro
 * experimental COB List Optimizations added (default: disabled)
 * new Function getSortRxCOB()
 *
 * Revision 2.72  2007/01/09 17:03:40  ro
 * documentation - doxygen groups added
 *
 * Revision 2.71  2006/12/21 14:35:03  ro
 * Bug FlushMbox() - BUFFER_INIT_PTR() call after msgIdentification(), now
 *                   This correct the problem, if msgIdentification()
 * 		  reset/change the RX Buffer.
 *
 * Revision 2.70  2006/11/03 13:37:42  ro
 * mode CONFIG_NO_GLOBAL_VARS added
 * Redundancy without global variables adapted
 * new function init_canDriverPtr()
 *
 * Revision 2.69  2006/08/03 12:43:36  ged
 * - typo in comment fixed
 *
 * Revision 2.68  2006/07/28 11:49:03  ro
 * Update v4.4 PIC18
 * Sync Counter in FlushMbox() corrected
 *
 * Revision 2.67  2006/06/29 14:03:04  boe
 * reorganize function and data declaration for usage of data structure with all global CANopen data
 *
 * Revision 2.66  2006/06/16 13:00:48  ro
 * multiline part of validCobId() corrected
 * (only wrong in the last internal version)
 *
 * Revision 2.65  2006/06/14 15:43:53  ro
 * Sync Consumer check in FlushMbox() added
 * (In a later version it will be possible to enable SYNC calculation in ISR or in FlushMbox().)
 * additional internal pointer typedefs for better programming
 * internal unused macros CONFIG_OLD_xxx changed to CONFIG_CAN_xxx
 * COB function better adapted for 8051 CPUs
 *
 * Revision 2.64  2006/03/31 15:34:56  ro
 * Redundancy Service added
 *
 * Revision 2.63  2006/03/21 17:18:22  ro
 * include examples.h only for specific test functionality
 *
 * Revision 2.62  2006/03/10 14:01:26  ro
 * Redundancy correction
 *
 * Revision 2.61  2006/02/23 14:28:07  ro
 * documentation corrected
 *
 * Revision 2.60  2006/02/10 10:10:49  ro
 * reference manual generation corrected
 *
 * Revision 2.59  2006/01/18 08:58:02  ro
 * COB functionality added
 * XDATA and NEAR usage redesigned
 * Multiline support added
 *
 * Revision 2.58  2005/12/16 13:32:25  ro
 * NEAR, CO_DATA added
 * validCobId() reworked
 *
 * Revision 2.57  2005/12/09 16:24:11  ro
 * validCobId() changed
 *
 * Revision 2.56  2005/07/14 09:12:14  ro
 * more changes for CANopen Library v4.4
 * most are: Buffer definition, FlushMbox()
 * CAN_Msg move to global memory, because some Compilers created wrong code
 * if it is local.
 *
 * Revision 2.55  2005/04/08 14:01:08  ro
 * CONFIG_TEST_VALID_COB changed to CONFIG_CAN_TEST_VALID_COB
 *
 * Revision 2.54  2005/03/10 14:54:20  ro
 * documentation added
 * own createCobIdIndex() function
 *
 * Revision 2.53  2005/03/08 16:46:57  ro
 * clean up for CANopen Library v4.4
 *
 *
 *
 *
...
*-- Allgemeine Funktionen fÅr den CAN-Treiber
*  \author port GmbH Halle (Saale)
*  $Revision: 2.93 $
*  $Date: 2013/08/26 12:24:17 $
*
...
*-- Treibern fÅr CAN Controller benutzt werden.
*
...
*-- und alle wÑhrend dieser Zeit zusÑtzlich empfangenen Messages
*-- verarbeitet.
*/
/**
...
*-- FlushMbox() sollte daher sehr hÑufig aufgerufen werden.
*/
/**
...
extern void usr_301_checkForEmcyMsgToSend(void);
...
*-- Tabelle aller in CANopen verfÅgbaren CAN-Bitraten.
...
*-- Summe aller benutzten COB EintrÑge.
...
*-- nÑchster freier Schreibindex im Receive Buffer 
*/
...
*-- nÑchster belegter Leseindex im Receive Buffer.
*/
...
*-- nÑchster freier Schreibindex im Transmit Buffer.
*/
...
-- nÑchster belegter Leseindex im Transmit Buffer
*/
...
*-- nÑchster freier Eintrag in ::cobList
*/
...
*-- Anzahl von COB EintrÑgen je CAN Linie
*/
#        ifdef CONFIG_DYN_MEM_ALLOC
...
*-- startoffset der COB EintrÑge je CAN Linie in ::cobList und ::cobIndexList 
*/
...
*-- \brief clearTxBuffer - lîscht den CAN Sendepuffer
* \ingroup bufferhandling
*
* \returns
*++ nothing
...
*-- \brief clearRxBuffer - lîscht den CAN Emfangspuffer
*
* \ingroup bufferhandling
*
*
...
*-- \brief checkTxBuffer - gibt den Status des Sendepuffers zurÅck
*
* \ingroup bufferhandling
*
*
...
*-- \brief Insert_TX_Request - fÅgt einen Transmission Request in den Puffer ein.
*
* \ingroup bufferhandling
* \retval CO_OK
*-- Nachricht in die Queue eingefÅgt
*++ Message was added to the queue
*
* \retval CO_E_CAN_TRANS_BUF
*-- Software-Puffer voll, Nachricht wurde nicht hinzugefÅgt
*++ software buffer full, message not added
*
* \retval CO_E_PARA_INCOMP
*-- Åbergebene Parameter fehlerhaft
...
*-- \brief checkBufferTimeout - prÅft alle timeout Zeiten im TXbuffer
*
*
* \returns 
* 0    - no timeout
...
*
*-- prÅfen, ob ein COB in den Index rein oder aus dem Index raus muss 
...
*-- validCobId() - prueft die GÅltigkeit der COB-IDs emfangener Nachrichten
*
*++ This function checks whether the COB-ID of a received message is valid
*++ for the local node.
*-- Diese Funktion prueft die GÅltigkeit der COB-IDs emfangener Nachrichten
*-- fÅr den localen Netzwerkknoten.
*
* \retval
...
*-- CANopen-Request
*-- erwartet.
*-- Der von der Empfangs Interruptservice Routine gefÅllte Puffer
*-- wird geleert und nacheinender immer eine Message
*-- zur Auswertung an eine interne CANopen Routine Åbergeben.
* 
...
*-- - CONFIG_FLUSHMBOX_READ_ALL\n
*--   alle, auch wÑhrend der Abarbeitung empfangenen Messages bearbeiten
* 
* \returns
*++ nothing
*-- nichts
...
#  endif /* CONFIG_DRIVER_TEST */
    ...
    CO_SET_BIT(1); /* time measurement start */
...
#  endif /* CONFIG_FLUSHMBOX_READ_ONLY_ONE */
    ...
    CO_RESET_BIT(1); /* time measurement end */
} /* void FlushMbox() */
#endif /* CONFIG_COLIB_FLUSHMBOX */
...
#  endif /* defined(CONFIG_MULT_LINES) || defined(CONFIG_REDUNDANCY_SUPPORT) */
	...
	{
	    /* Msg for this node */
		PRINTF(" %s%x:%d: \n", s, (int)pCAN_Msg->cobId, (int)pCAN_Msg->length);
    } else
	{
...
#  endif /* CONFIG_REDUNDANCY_SUPPORT */
    ...
    p_cobList[0] = malloc(sizeof(COB_T) * cobCnt);
    *pCobList = p_cobList[0];
    co_maxCobCnt = cobCnt;

#  ifdef CONFIG_DRIVER_FAST_SORT
    p_cobIndexList[0] = malloc(sizeof(COB_ARRAY_INDEX_T) * cobCnt);
